...
| 字段名 | 类型 | 必填 | 说明 | 示例 |
|--------|------|------|------|
| `row_filter` | object | ❌ | 行过滤配置，用于排除特定行（如"合计"行） | 见下方 |
...
`row_filter` 用于自动过滤掉输入Excel中的特定行（如汇总行、标题行等）。

### 配置结构

```json
"row_filter": {
  "exclude_keywords": ["关键字1", "关键字2"]
}
```
...
```
Row 1: 表头
Row 2: 汪辉, 622848..., 5022.01
Row 3: 王春妍, 622848..., 4286.24
Row 4: 王昳丽, 622848..., 4286.24
Row 5: 合计, , 16782.08  ← 被过滤
```
