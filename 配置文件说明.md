...
## default 和 crossbank（多规则组配置）

`default` 和 `crossbank` 等字段是**规则组配置**，用于定义不同模板的数据处理规则。
...
**工作原理**：
1. 当启用 `template_selector` 时，系统会根据"开户银行"字段将数据分组
2. 每个分组使用对应的规则组配置：
   - **default** 组：使用 `default` 规则组
   - **crossbank** 组：使用 `crossbank` 规则组
3. 如果规则组中不存在某个配置项，则使用单位配置中的值作为回退

**配置说明**：

规则组中可以包含以下字段（所有字段都是可选的，如不存在则使用单位配置中的值）：

| 字段名 | 类型 | 必填 | 说明 |
|--------|------|------|
| `template_path` | string | ✅ | 模板文件路径 | `templates/default_template.xlsx"` |
| `header_row` | integer | ✅ | 表头所在行号 | `1` |
| `start_row` | integer | ❌ | 数据起始行号，默认为 `header_row + 1` | `2` |
| `field_mappings` | object | ✅ | 字段映射配置 | 见下方 |
| `transformations` | object | ✅ | 数据转换配置 | 见下方 |
| `row_filter` | object | ❌ | 行过滤配置 | 见上方 |
| `fixed_values` | object | ❌ | 固定值配置 | 见下方 |
| `auto_number` | object | ❌ | 自动编号配置 | 见下方 |
| `month_type_mapping` | object | ❌ | 月份类型映射配置 | 见下方 |

**注意**：
- 规则组中的配置项优先级高于单位配置
- 如果规则组中不存在某个配置项，会使用单位配置中的值
- 这允许在不同规则组之间共享一些通用配置
    ...
    "单位名称": {
      ...
      "跨行": {
        ...
        "field_mappings": {
          ...
          "开户银行（行别）": {
            "source_column": "开户银行",
            "target_column": "开户银行（行别）",
            "transform": "none",
            "required": false
          },
          "开户行支行名称": {
            "source_column": "开户银行",
            "target_column": "开户行支行名称",
            "transform": "none",
            "required": false
          }
        },
...
**注意事项**：
- 规则组的键名（如 `default`、`crossbank`）可以自定义，但需要与 `template_selector` 配置保持一致
- 如果未启用 `template_selector`，系统会使用单规则结构（向后兼容）
- 规则组配置可以只包含需要覆盖的字段，其他字段使用单位配置中的值
