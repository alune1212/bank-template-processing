...
def _create_test_templates(tmp_path: Path) -> tuple[Path, Path]:
    ...
    return default_template, crossbank_template


def _make_month_type_mapping(enabled: bool) -> dict[str, object]:
    return {
        "enabled": enabled,
        "target_column": "用途",
...
def _make_merge_test_group_config(template_path: Path, *, month_type_enabled: bool = False) -> dict[str, Any]:
    return make_basic_unit_config(
        template_path=str(template_path),
        field_mappings={
            "姓名": make_field_mapping(
                source_column="姓名",
                target_column="姓名",
                transform="none",
            ),
        ...
        validation_rules={},
        month_type_mapping=_make_month_type_mapping(month_type_enabled),
    )
...
def _build_test_config(
    ...
    return make_config(
        ...
        unit_config=make_multi_group_unit_config(
            {
                "default": _make_merge_test_group_config(default_template, month_type_enabled=month_type_enabled),
                "crossbank": _make_merge_test_group_config(crossbank_template, month_type_enabled=month_type_enabled),
            },
            template_selector={
                "enabled": True,
                "default_bank": "农业银行",
...
def _write_config_file(tmp_path: Path, config: dict[str, Any]) -> Path:
...
def _call_prepare_merge_tasks(
    ...
    config: dict[str, Any],
    *,
    needs_month_for_filename: bool = False,
...
def test_resolve_rule_group_prefers_group_name_then_template_stem(tmp_path):
    default_template, crossbank_template = _create_test_templates(tmp_path)
...
def test_prepare_merge_tasks_stats_mismatch_raises(tmp_path):
    ...
    with pytest.raises(MergeFolderError, match="人数校验失败"):
        _call_prepare_merge_tasks(merge_dir, config)
...
def test_prepare_merge_tasks_orders_by_mtime_not_filename(tmp_path):
    ...
    tasks = _call_prepare_merge_tasks(merge_dir, config)
...
def test_prepare_merge_tasks_skips_month_inference_when_filename_template_not_use_month(tmp_path, caplog):
    ...
    tasks = _call_prepare_merge_tasks(merge_dir, config, logger=logging.getLogger(__name__))
