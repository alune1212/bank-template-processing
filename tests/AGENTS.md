...
## OVERVIEW
基于 `pytest`/`pytest-cov` 的完整测试套件，覆盖配置加载、主流程编排、运行时路径保护、共享管线、Excel 双格式读写、数据转换/校验、模板分组、批量合并以及文档同步。
...
| 读取策略回归 | `test_excel_reader.py` / `test_excel_reader_error_paths.py` | `.xlsx/.xls`、过滤、`reader_options.header_row` |
| 公式缓存行为 | `test_excel_reader_formula.py` | `data_only=True/False` 差异 |
| 写入复杂路径 | `test_excel_writer.py` / `test_excel_writer_error_paths.py` | `clear_rows`、列解析、XLS/XLSX 边界 |
...
- `tests/test_excel_writer.py` 与 `tests/test_excel_writer_consistency.py` 对 `clear_rows` 在 `.xlsx/.xls` 中的关键行为都有覆盖。
